...
def test_openrouter_payload_repeats_bounded_runtime_contract(monkeypatch, tmp_path):
    ...
    assert "Read `decision_signals.allowed_frontier_ids`." in developer_text
    assert "Never use the order of actions in the goal text as evidence." in developer_text
    assert "same issue still persists or is not improving" in developer_text
    assert "Treat `decision_signals.vision_signal`, `decision_signals.notebook_notes`, and `last_result_notes`" in developer_text
    assert "Use `last_result_notes` and `decision_signals.symptom_feedback` together." in developer_text
    assert "Use `temporal_action_context` to avoid repeating a very recent action only inside the short settle window." in developer_text
    assert "Use `freshness` to discount stale or still-settling observations" in developer_text
    assert "During healthy print startup, if only pause or cancel remain" in developer_text
    assert "Near print finish, if extrusion-related symptoms are still present" in developer_text
