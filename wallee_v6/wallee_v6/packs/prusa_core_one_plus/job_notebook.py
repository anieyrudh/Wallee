...
CURRENT_NOTEBOOK_SCHEMA_VERSION = "1.0"
CURRENT_NOTEBOOK_PARSER_VERSION = "job_notebook_v2_bgcode_decode"
...
_PRINT_TIME_RE = re.compile(r"(\d+)h\s*(\d+)m", re.IGNORECASE)
...
def notebook_from_jsonable(payload: dict[str, Any]) -> JobNotebook:
    ...
    if payload.get("schema_version") != CURRENT_NOTEBOOK_SCHEMA_VERSION:
    ...
    _require_exact_keys(baselines_payload, _NOTEBOOK_BASELINE_KEYS, "notebook.baselines")
    ...
    for section in sections:
        ...
        notes.append(
            NotebookNote(
                ...
                reason_tags=tuple(sorted(section.tags)),
...
def _note_from_json(raw_note: Any) -> NotebookNote | None:
    ...
    suggested_families = tuple(
        ...
        for family in (
            ...
            if str(item) in {"speed", "flow", "nozzle", "bed"}
        )
    )
...
def _section_from_jsonable(payload: Any) -> JobSection:
    ...
    _require_exact_keys(payload, _NOTEBOOK_SECTION_KEYS, "notebook.sections[]")
