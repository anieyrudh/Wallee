...
that class; these run `python -m wallee.main` for real and assert
...
def _run_cli(data_dir: Path, *extra_args: str) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "wallee.main", "--simulate", "--goal", GOAL, *extra_args],
        cwd=REPO_V6,
        env=env,
        capture_output=True,
        text=True,
...
def _db(data_dir: Path) -> sqlite3.Connection:
    conn = sqlite3.connect(data_dir / "wallee.sqlite3")
...
def test_cli_boot_reconcile_is_wired_through_the_real_entrypoint(tmp_path):
    ...
    try:
        ...
        from wallee.models import ActionRun, ActionRunStatus, HazardClass, PlanRecord
