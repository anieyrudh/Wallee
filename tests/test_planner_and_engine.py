...
from wallee.predicates import PredicateEvaluator, atom
...
def test_wait_until_uses_short_slice_per_cycle(tmp_path, monkeypatch):
    ...
    try:
        with patch("wallee.engine.time.monotonic", side_effect=lambda: next(monotonic_values)):
            with patch("wallee.engine.time.sleep", side_effect=lambda seconds: sleep_calls.append(seconds)):
                result = engine._execute_builtin(action, "Wait for cooldown")
    finally:
        engine.close()
        runtime_db.close()
