...
@tool(kind="actuator", requires_approval=False)
def remember(observation: str) -> dict:
    """Persist a concise observation to OBSERVATIONS.md."""

@tool(kind="actuator", requires_approval=False)
def lookup_issue(query: str) -> dict:
    """Search REFERENCE.md for diagnosis and intervention guidance."""
...
`remember()` appends a bounded observation log.
`lookup_issue()` returns targeted guidance from REFERENCE.md.
...
class AgentLoop:
    ...
    def run(self):
        while True:
            ...
            state = self.wb.read_all_with_annotations()
...
### 6.3 Context builder — trend annotation

The context builder computes trend annotation directly (not via LLM tools):
...
This runs inside `read_all_with_annotations()` — pure Python, no LLM call.
...
class Whiteboard:
    ...
    def read_all_with_annotations(self) -> dict:
...
### 12.3 LEARNED.md
Accumulated wisdom. Initially manual. Updated by the operator or by the `remember()` tool.
...
wallee/
    ...
    ledger/
        ...
        migrations/
            001_init.sql
    whiteboard/
        client.py             # Redis client with TTL + ring buffers + trend annotation
    bus/
        gpio.py
        i2c.py
        spi.py
...
3. Implement `whiteboard/client.py` (Redis wrapper with TTL, ring buffers, trend annotation)
...
- Whiteboard has host_pi sensor data with trend annotation
...
9. Implement built-in tools: `web_search`, `discover_hardware`, `call_human`, `remember`, `lookup_issue`, `get_sensor_history`
...
8. **Trend annotation is computed by deterministic code, not the LLM.**
9. **Raw history stays available.** Detailed inspection comes from `get_sensor_history()`.
