...
class TestBuddyDiscovery:
    ...
    def test_finds_cameras_by_mac(self, mock_run):
        mock_run.return_value = type("R", (), {
            "stdout": "192.0.2.194 dev eth0 lladdr 88:49:2d:00:00:01 REACHABLE\n"
                      "192.0.2.200 dev eth0 lladdr 88:49:2d:00:00:02 STALE\n"
                      "192.0.2.10 dev eth0 lladdr aa:bb:cc:dd:ee:ff REACHABLE\n",
            "returncode": 0,
        })()
        ips = discover_buddy_cameras()
        assert "192.0.2.194" in ips
    ...
    def test_no_cameras(self, mock_run):
        mock_run.return_value = type("R", (), {
            "stdout": "192.0.2.10 dev eth0 lladdr aa:bb:cc:dd:ee:ff REACHABLE\n",
            "returncode": 0,
        })()
        ips = discover_buddy_cameras()
        assert ips == []
...
class TestBuddyCameraSensor:
    ...
    @patch("wallee.device_packs.pi_cameras.sensors.discover_buddy_cameras", return_value=["192.0.2.194"])
    def test_one_buddy(self, mock_disc, mock_rtsp):
        result = read_buddy_cameras()
        assert result["camera.buddy_count"] == 1
        assert result["camera.buddy1_status"] == "live"
        assert result["camera.buddy1_ip"] == "192.0.2.194"
